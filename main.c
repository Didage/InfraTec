...
void codificar(ARCHIVO *arch, ARCHIVO *resultado,  unsigned short claveBinaria, int tamanhoClave)
{
	...
	if(tamanhoClave<8) {
		...
		//construye un arreglo de carateres con la porción de clave correcta segun cada carater del mensaje y lo almacena en claveCompilada
		for (int i = 0; i < tamanhoArchivoLectura; i++)	{

			// caracter vacío donde se contruye el caracter con la porción de la clave correspondiente al carater i del mensaje
			unsigned char claveLocal = '\0';
			unsigned char claveAux = claveBinaria;

			// Copia al comienzo los dígitos binarios que se descuadra por el tamaño de la clave
			claveLocal = append(offset, claveAux, claveLocal);

			// Copia la clave tantas veces cabe en 
			for (int j = 0; j < 8/tamanhoClave; j++) {
				claveLocal = copy(claveBinaria, claveLocal);
			}

			// Copia la porción desfazada de la clave al final de lo que se lleva
			if(8%tamanhoClave!=0) {
				claveAux = claveAux >> (tamanhoClave-((8%tamanhoClave)+offset));
				claveLocal = copy(claveAux, claveLocal);
			}

			// Lleva la cuenta del desfase en las claves
			if(8%tamanhoClave!=0) {
				offset++;
	...
	// Lo mismo, pero en el caso en que la clave tiene tamaño mayor a 8
	if(tamanhoClave>8) {
		...
		for (int i = 0; i < tamanhoArchivoLectura/2; i++)	{
			...
			// En este caso los desfases van multiplicados por cuanto sobrepasa el tamaño de char (variable diff)
		...
		// Se agrega el último char en caso de claves con longitud inpar
	...
	// En el caso en que la clave tiene tamaño 8 se crea el vector directamente
	if (tamanhoClave==8) {
		for (int i = 0; i < tamanhoArchivoLectura; i++) {
	...
	// Aplica el XOR con cada char de la información leída con respecto al arreglo 
	for (int i = 0; i < tamanhoArchivoLectura; ++i) {
